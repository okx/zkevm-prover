...
void Starks::genProof(FRIProof &proof, Goldilocks::Element *publicInputs, Goldilocks::Element verkey[4], CHelpersSteps *chelpersSteps)
{
    ...
    if (ncols > 0)
    {
        ntt.LDE_MerkleTree_Auto(treesGL[0]->get_nodes_ptr(), p_cm1_n, N, NExtended, ncols, reduceMemory?p_cm1_2ns_tmp?p_cm1_2ns, reduceMemory?3:0);
    }
    else
    {
        treesGL[0]->merkelize();
...
#pragma omp parallel for num_threads(nthreads)
    ...
    if (ncols > 0)
    {
        ntt.LDE_MerkleTree_Auto(treesGL[1]->get_nodes_ptr(), p_cm2_n, N, NExtended, ncols, reduceMemory?p_cm2_2ns_tmp:p_cm2_2ns);
    }
    else
    {
        treesGL[1]->merkelize();
